  ...
  return EvaluationRunSummarySchema.parse({
    ...
    runtime: {
      ...
      models: {
        ...
        resolution: row.generationModel,
...
function caseFromRow(row: CaseRow): PersistedEvaluationCase {
  return PersistedEvaluationCaseSchema.parse({
    ...
    telemetry: {
      ...
      classificationInputTokens: row.classificationInputTokens ?? 0,
      classificationOutputTokens: row.classificationOutputTokens ?? 0,
      classificationCachedInputTokens: row.classificationCachedInputTokens ?? 0,
      classificationCacheWriteTokens: row.classificationCacheWriteTokens ?? 0,
      resolutionInputTokens: row.resolutionInputTokens ?? 0,
      resolutionOutputTokens: row.resolutionOutputTokens ?? 0,
      resolutionCachedInputTokens: row.resolutionCachedInputTokens ?? 0,
      resolutionCacheWriteTokens: row.resolutionCacheWriteTokens ?? 0,
...
function valuesForRun(run: PersistedEvaluationRun) {
  return {
    ...
    classificationModel: run.runtime.models?.classification ?? run.runtime.model ?? "legacy",
    generationModel: run.runtime.models?.resolution ?? run.runtime.model ?? "legacy",
...
function valuesForCases(run: PersistedEvaluationRun) {
  return run.cases.map((item) => ({
    ...
    classificationInputTokens: item.telemetry.classificationInputTokens ?? 0,
    classificationOutputTokens: item.telemetry.classificationOutputTokens ?? 0,
    classificationCachedInputTokens: item.telemetry.classificationCachedInputTokens ?? 0,
    classificationCacheWriteTokens: item.telemetry.classificationCacheWriteTokens ?? 0,
    resolutionInputTokens: item.telemetry.resolutionInputTokens ?? 0,
    resolutionOutputTokens: item.telemetry.resolutionOutputTokens ?? 0,
    resolutionCachedInputTokens: item.telemetry.resolutionCachedInputTokens ?? 0,
    resolutionCacheWriteTokens: item.telemetry.resolutionCacheWriteTokens ?? 0,
