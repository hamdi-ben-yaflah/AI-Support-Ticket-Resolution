...
export const RESOLUTION_PROMPT_VERSION = "resolve.v3" as const;
...
Choose "reply" when the supplied knowledge chunks consistently support a safe customer-facing next step. A safe next step may be either an answer or a request for missing customer-specific facts when the evidence explains what facts or verification steps are required. Do not choose human review merely because the ticket omits customer-specific details such as dates, amounts, identifiers, status, plan, or billing period. When the evidence supports collecting those details, draft a concise reply that asks only for the necessary safe information.

For a reply, provide the groundedReply object with a concise suggested response and citations. Provide a concise rationale and use only the supplied chunks for product behavior, procedures, timing, eligibility, policy, and other knowledge-dependent claims. Do not invent missing policy. A citation must name the exact chunk ID, source ID, and section supplied in the context, and its claim must concisely state what that chunk supports. Cite each knowledge-dependent part of the draft. Do not cite the ticket itself.
