...
import { PersistedResolutionRunSchema, ResolutionExecutionSchema } from "@/domain/resolution-run";
...
describe("TicketInputSchema", () => {
  it("trims valid input and accepts both documented tiers", () => {
    expect(TicketInputSchema.parse({ text: "  Charged twice  ", customerTier: "premium" })).toEqual(
      { text: "Charged twice", customerTier: "premium" },
    );
    expect(
      TicketInputSchema.safeParse({ text: "Password reset", customerTier: "standard" }).success,
    ).toBe(true);
  });
  ...
  it("accepts exact text boundaries and rejects unsupported tiers", () => {
    ...
    expect(
      TicketInputSchema.safeParse({ text: "A valid support ticket", customerTier: "vip" }).success,
    ).toBe(false);
  });
});
  ...
  it("rejects a non-UUID trace and unknown error code", () => {
    expect(
      schema.safeParse({
        ok: false,
        traceId: "trace",
        error: { code: "secret", message: "x", retryable: false },
      }).success,
    ).toBe(false);
  });
});
...
describe("grounded reply schemas", () => {
  ...
  it("accepts a bounded cited reply and both proposal branches", () => {
    ...
    expect(
      ResolutionProposalSchema.safeParse({
        category: "billing",
        priority: "medium",
        summary: "Duplicate charge",
        confidence: 0.9,
        action: "reply",
        reason: "The policy supports a review.",
        groundedReply,
      }).success,
    ).toBe(true);
    expect(
      ResolutionProposalSchema.safeParse({
        ...
        reason: "No relevant policy was retrieved.",
      }).success,
    ).toBe(true);
  });

  it("requires citations with UUIDs and enforces response and citation limits", () => {
    expect(GroundedReplySchema.safeParse({ suggestedResponse: "x", citations: [] }).success).toBe(
      false,
    );
    expect(
      GroundedReplySchema.safeParse({
        suggestedResponse: "x".repeat(4_001),
        citations: groundedReply.citations,
      }).success,
    ).toBe(false);
    expect(
      GroundedReplySchema.safeParse({
        suggestedResponse: "x",
        citations: Array.from({ length: 9 }, () => groundedReply.citations[0]),
      }).success,
    ).toBe(false);
    expect(
      GroundedReplySchema.safeParse({
        suggestedResponse: "x",
        citations: [{ ...groundedReply.citations[0], chunkId: "not-a-uuid" }],
      }).success,
    ).toBe(false);
  });
  ...
  it("rejects hybrid, unbounded, and unsupported proposal actions", () => {
    ...
    expect(
      ResolutionProposalSchema.safeParse({ ...base, action: "reply", reason: "Review it." })
        .success,
    ).toBe(false);
    expect(
      ResolutionProposalSchema.safeParse({ ...base, action: "escalate", reason: "Review it." })
        .success,
    ).toBe(false);
    expect(
      ResolutionProposalSchema.safeParse({
  ...
  it("keeps retrieval unavailability as an API error and rejects insufficient evidence", () => {
    ...
    expect(
      schema.safeParse({
        ok: false,
        traceId,
        error: { code: "retrieval_unavailable", message: "Safe message", retryable: true },
      }).success,
    ).toBe(true);
    expect(
      schema.safeParse({
        ok: false,
        traceId,
        error: { code: "insufficient_evidence", message: "Safe message", retryable: false },
      }).success,
    ).toBe(false);
  });
});
...
describe("refund-review schemas", () => {
  ...
  it("accepts only bounded unique display-safe action arguments", () => {
    ...
    expect(
      RequestRefundReviewArgsSchema.safeParse({
        ...arguments_,
        reason: "too short",
      }).success,
    ).toBe(false);
    expect(
      RequestRefundReviewArgsSchema.safeParse({
        ...arguments_,
        evidenceChunkIds: [arguments_.evidenceChunkIds[0], arguments_.evidenceChunkIds[0]],
      }).success,
    ).toBe(false);
    expect(
      RequestRefundReviewArgsSchema.safeParse({
  ...
  it("requires the allowlisted pending proposal and exact positive confirmation", () => {
    ...
    expect(
      RefundReviewActionProposalSchema.safeParse({
        ...actionProposal,
    ...
    expect(RefundReviewConfirmationSchema.safeParse({ confirmed: true, force: true }).success).toBe(
      false,
    );
  });
  ...
  it("accepts only a clearly labeled bounded local mock result", () => {
    expect(
      MockRefundReviewResultSchema.safeParse({
  ...
  it("allows refund review only for billing with matching citations and arguments", () => {
    ...
    expect(ResolutionProposalSchema.safeParse({ ...proposal, category: "technical" }).success).toBe(
      false,
    );
    expect(
      ResolutionProposalSchema.safeParse({
...
describe("source and resolution-run schemas", () => {
  ...
  it("allows only bounded display-safe source fields", () => {
    ...
    expect(SourceDetailSchema.safeParse({ ...source, content: "x".repeat(20_001) }).success).toBe(
      false,
    );
  });
  ...
  it("validates a successful run without raw ticket text", () => {
    ...
    expect(PersistedResolutionRunSchema.safeParse({ ...run, ticketText: "raw" }).success).toBe(
      false,
    );
    expect(
      PersistedResolutionRunSchema.safeParse({
        ...run,
        metadata: { ...run.metadata, latencyMs: -1 },
      }).success,
    ).toBe(false);
  });
  ...
  it("requires cited snapshots to match public citations in the same order", () => {
    ...
    expect(
      ResolutionExecutionSchema.safeParse({
        ...execution,
  ...
  it("requires zero sources for human review and at least one for reply runs", () => {
    ...
    expect(
      ResolutionExecutionSchema.safeParse({ proposal, citedSources: [], metadata }).success,
    ).toBe(true);
    expect(
      ResolutionExecutionSchema.safeParse({
        proposal,
    ...
    expect(
      PersistedResolutionRunSchema.safeParse({
        ...persistedBase,
        action: { type: "needs_human_review", reason: proposal.reason },
        citedSources: [],
      }).success,
    ).toBe(true);
    expect(
      PersistedResolutionRunSchema.safeParse({
        ...persistedBase,
        action: { type: "reply", reason: "Supported." },
        citedSources: [],
      }).success,
    ).toBe(false);
    expect(
