...
const evidence: RetrievedEvidence[] = [{ chunkId, sourceId: "duplicate-charges", section: "Duplicate charges > Review", content: "Settled duplicate charges may be submitted for review.", tokenCount: 10, similarity: 0.9 }];
...
describe("grounded resolution", () => {
  ...
  it("combines the original classification with a validated grounded reply", async () => {
    const classifier = vi.fn().mockResolvedValue(classification);
    const evidenceRetriever = retriever();
    await expect(resolveTicket({ text: "I was charged twice." }, { traceId, classifier, retriever: evidenceRetriever, provider: provider(reply), log: logger() })).resolves.toEqual({ ...classification, groundedReply: reply });
  ...
  it.each([
    ...
    await expect(resolveTicket({ text: "I was charged twice." }, { traceId, classifier: vi.fn().mockResolvedValue(classification), retriever: retriever(), provider: provider(invalidReply), log: logger() })).rejects.toMatchObject({ code: "invalid_output" });
  });
  ...
  it("does not generate when evidence is insufficient", async () => {
    ...
    await expect(resolveTicket({ text: "Unsupported question" }, { traceId, classifier: vi.fn().mockResolvedValue(classification), retriever: noEvidence, provider: llm, log: logger() })).rejects.toMatchObject({ code: "insufficient_evidence" });
