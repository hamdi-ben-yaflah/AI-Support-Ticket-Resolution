...
describe("retrieval", () => {
  ...
  it("uses the category search without fallback when evidence is adequate", async () => {
    ...
    await expect(retrieveEvidence({ text: "duplicate charge", category: "billing", traceId: "trace" }, { embedder, search, config, log: { info: vi.fn(), warn: vi.fn(), error: vi.fn() } })).resolves.toHaveLength(1);
  ...
  it("broadens when category evidence is inadequate", async () => {
    const search = vi.fn().mockResolvedValueOnce([candidate(0, 0.2)]).mockResolvedValueOnce([candidate(1, 0.9)]);
    const result = await retrieveEvidence({ text: "question", category: "other", traceId: "trace" }, { embedder, search, config, log: { info: vi.fn(), warn: vi.fn(), error: vi.fn() } });
    expect(result[0]?.chunkId).toBe(ids[1]);
    expect(search).toHaveBeenNthCalledWith(2, expect.not.objectContaining({ category: expect.anything() }));
  });

  it("returns controlled insufficient evidence and unavailable failures", async () => {
    const log = { info: vi.fn(), warn: vi.fn(), error: vi.fn() };
    await expect(retrieveEvidence({ text: "unknown", category: "other", traceId: "trace" }, { embedder, search: vi.fn().mockResolvedValue([]), config, log })).rejects.toMatchObject({ code: "insufficient_evidence" });
    await expect(retrieveEvidence({ text: "unknown", category: "other", traceId: "trace" }, { embedder, search: vi.fn().mockRejectedValue(new Error("db")), config, log })).rejects.toMatchObject({ code: "unavailable" });
  });
});
